...
namespace PlatinDashboard.Presentation.MVC.Models
{
    ...
    public class Exemplos
    {
        ...
        public BoletoBancario Bradesco(decimal valor, string codregional, string nomregional)
        {
            string documento = codregional + DateTime.Now.ToString("MMyyy");// "01000000001";
            string cnpjregional = "000.000.000-00";
            DateTime vencimento = DateTime.Now.AddDays(10);

            Instrucao_Bradesco item = new Instrucao_Bradesco(9, 5); ;

            Cedente c = new Cedente("47.331.822/0001-19", "ASSOCIACAO PAULISTA DE CIRURGIOES DENTISTAS", "0091", "0", "0052974", "9");
            c.Codigo = "88888";

            EnderecosNeg enderecosNeg = new EnderecosNeg();
            var enderecos = enderecosNeg.BuscaPorCodigo(codregional);
            //Carteiras 
            string nossonumero = codregional + DateTime.Now.ToString("MMyyyy");
            nossonumero = nossonumero.PadLeft(11, '0');
            BoletoNet.Boleto b = new BoletoNet.Boleto(vencimento, valor, "09", nossonumero, c);//"00290000034"
            b.ValorMulta = 0.10m;
            b.ValorCobrado = valor;// 1.10m;
            b.NumeroDocumento = documento;
            b.DataVencimento = new DateTime(2015, 09, 12);

            b.Sacado = new Sacado(cnpjregional, nomregional);
            b.Sacado.Endereco.End = enderecos[0].Descricao;// "Endereço do seu Cliente ";
            b.Sacado.Endereco.Bairro = enderecos[0].Bairro;// "Bairro";
            b.Sacado.Endereco.Cidade = enderecos[0].Cidade;// "Cidade";
            b.Sacado.Endereco.CEP = enderecos[0].Cep;// "00000000";
            b.Sacado.Endereco.UF = enderecos[0].UF;// "UF";
