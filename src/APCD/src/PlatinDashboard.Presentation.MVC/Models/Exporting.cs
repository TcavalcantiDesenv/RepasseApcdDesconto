        ...
        public void ExportDetails(DataTable DetailsTable, string FormatType, string FileName)
        {
            try
            {
                ...
                for (int i = 0; i < dtExport.Columns.Count; i++)
                {
                    ...
                    cabeca = dtExport.Columns[i].ColumnName.Replace(@"""", "");
                    linha = ReplaceSpclChars(dtExport.Columns[i].ColumnName.Replace(@"""", ""));
                    //cabeca = cabeca.Replace(@"""", "");
                    //linha = linha.Replace(@"""", "");
                    //linha = linha + ";"+ linha;
                    //cabeca = cabeca + ";" + cabeca;
                    sHeaders[i] = cabeca;// dtExport.Columns[i].ColumnName.Replace("\"","");
                    sFileds[i] = linha;// ReplaceSpclChars(dtExport.Columns[i].ColumnName.Replace("\"", ""));
                }
                arquivo = cabeca + linha;
                if (appType == "Web")
